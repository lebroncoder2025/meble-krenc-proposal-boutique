...
const key='mk-boutique-privacy-v2',notice=document.querySelector('.cookie-notice'),settings=document.querySelector('.cookie-dialog');
let saved=false;try{saved=localStorage.getItem(key)==='saved';}catch{}
if(notice)notice.hidden=saved;
document.querySelectorAll('[data-cookie-open]').forEach(b=>{b.hidden=false;b.addEventListener('click',()=>{settings.showModal();document.body.classList.add('modal-open');});});
document.querySelectorAll('[data-cookie-save]').forEach(b=>b.addEventListener('click',()=>{try{localStorage.setItem(key,'saved');}catch{}if(notice)notice.hidden=true;settings?.close();}));
document.querySelector('[data-cookie-reset]')?.addEventListener('click',()=>{try{localStorage.removeItem(key);}catch{}settings.querySelector('.storage-status').textContent='Zapis ustawień został usunięty.';if(notice)notice.hidden=false;});
